      ...
      expect(
        (
          await consumeRateLimit(
            ctx.db,
            rateLimitKeys.import("ana"),
            RATE_LIMITS.importPerUser,
            now,
          )
        ).allowed,
      ).toBe(true);
  ...
  it("limits checkouts per IP hash and list", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      await enforceRateLimit(
        ctx.db,
        rateLimitKeys.checkout(ip, "list-a"),
    ...
    expect(error).toMatchObject({
      code: "RATE_LIMITED",
      status: 429,
      extra: { retryAfterSeconds: 600 },
    });

    // Another list, or another IP, has its own budget.
    await enforceRateLimit(
      ctx.db,
      rateLimitKeys.checkout(ip, "list-b"),
      RATE_LIMITS.checkoutPerIpAndList,
      now,
    );
    ...
    await enforceRateLimit(
      ctx.db,
      rateLimitKeys.checkout(otherIp, "list-a"),
      RATE_LIMITS.checkoutPerIpAndList,
      now,
    );
  });
  ...
  it("never exceeds the limit under concurrency", async () => {
    ...
    const results = await Promise.all(
      Array.from({ length: 25 }, () =>
        consumeRateLimit(ctx.db, key, { limit: 10, windowMs: 60_000 }),
      ),
    );
...
describe("audit trail", () => {
  it("records publication without share tokens or secret links", async () => {
    ...
    const { shareToken } = await publishList(ctx.deps, {
      listId: list.id,
      ownerId: owner.id,
      visibility: "UNLISTED",
    });
